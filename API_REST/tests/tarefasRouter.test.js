...
describe('Testes do recurso /tarefas', () => {
    ...
    test('POST / deve retornar 422 (nome curto)', async () => {
        const response = await request
            .post(url)
            .set("Authorization", `Bearer ${token}`)
            .send({ nome: "a" });
    ...
    test('POST / deve retornar 422 (nome vazio)', async () => {
        const response = await request
            .post(url)
            .set("Authorization", `Bearer ${token}`)
            .send({ nome: "   " });
    ...
    test('PUT / id deve retornar 422 (nome vazio)', async () => {
        const response = await request
            .put(`${url}/${id}`)
            .set("Authorization", `Bearer ${token}`)
            .send({ nome: "   " });
    ...
    test('PUT / id deve retornar 422 (nome curto)', async () => {
        const response = await request
            .put(`${url}/${id}`)
            .set("Authorization", `Bearer ${token}`)
            .send({ nome: "a" });
